def tromino(board, srow, scol, size, xrow, xcol):
    ...
    if xrow < mrow and xcol < mcol: # 1사분면
        fillCenterExcept(board, mrow, mcol, 1) # 해당 분면을 제외하고 mrow, mcol과 인접한 부분 칠해주기
    ...
    tromino(board, srow, scol, size//2, xrow1, xcol1) # 다시 싹다 나눠서 칠해주기: 재귀 호출
...
def print_board(board): # 보드 출력 함수
    for i in range(m):
        for j in range(m):
            if board[i][j] < 0:
                print("%3s"%"X", end="")
...
import random
m = 8
xrow = random.randint(0, m-1) # 임의로 x 위치 선정
